...
class TutorRequests extends BaseRequest {
  constructor() {
    super();
  ...
  forgotPasswordRequest() {
    return [
      check('email').trim()
        ...
        .custom(async (email) => {
          if (await this.tutorRepository.checkExist({ email })) {
            return Promise.resolve();
          }
  ...
  getEmailExistRule(withUser) {
    return check('email').trim()
      ...
      .custom(async (email, { req }) => {
        ...
        if (await this.tutorRepository.verifyEmailSlug(email, userId)) {
          return Promise.reject();
        }
        return Promise.resolve();
      });
  ...
  getOldPasswordRule() {
    return this.getPasswordRule(true, 'currentPassword').custom(async (value, { req }) => {
      ...
      const { password } = await this.tutorRepository.getById(id, ['password']);
