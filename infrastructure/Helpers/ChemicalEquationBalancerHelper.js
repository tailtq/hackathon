      ...
      for (var _i = 0, terms_1 = terms; _i < terms_1.length; _i++) {
        ...
        if (coef != 0) {
          if (head)
            ...
            node += " + ";
          if (coef != 1) {
            node += coef.toString().replace(/-/, MINUS);
          }
...
var Group = /** @class */ (function () {
  ...
  Group.prototype.toHtml = function () {
    ...
    node += ")";
    if (this.count != 1)
      node += `<sub>${this.count.toString()}</sub>`;
    return node;
  };
  return Group;
}());
...
export default formulaStr => {
// module.exports = formulaStr => {
  try {
    const eqn = new Parser(formulaStr).parseEquation();
    const matrix = buildMatrix(eqn);
    solve(matrix);
