...
class UserRequests extends BaseRequest {
  ...
  forgotPasswordRequest() {
    return [
      check('email').trim()
        ...
        .custom(async (email) => {
          if (await this.repository.checkExist({ email })) {
            return Promise.resolve();
          }
  ...
  getEmailExistRule(withUser) {
    return check('email').trim()
      ...
      .custom(async (email, { req }) => {
        ...
        if (await this.repository.verifyEmailSlug(email, userId)) {
          return Promise.reject();
        }
        return Promise.resolve();
      });
  ...
  getNameRule() {
    return check('name').trim().isString().not().isEmpty().isLength({ min: 1, max: 120 });
  }
  ...
  getOldPasswordRule() {
    return this.getPasswordRule(true, 'currentPassword').custom(async (value, { req }) => {
      ...
      const { password } = await this.repository.getById(id, ['password']);
